...
function Verlet() {
	...
	*/
	...
	/* 	
	 *	Predefined Functions For Creating Simple Objects Like (box,rope etc)
	 *	@method Verlet.Poly
	*/
	this.tearArray = [];
	this.Poly = {
		...
		*/	
		...
		/* 	
		 *	Create A Awesome Cloth
		 *	@method Verlet.Poly.cloth()
		 *	@param {object} opt
		 *	@param [Array] dots
		 *	@param [Array] cons
		*/		
		advanceCloth : function createCloth(opt,dot,con) {
				let dots = dot || this.dots;
				let cons = con || this.cons;
				let delta = 1;
				let pls = dots.length;
			...
			/* 	
			*	Create A Simple Cloth (But Accurate)
			*	@method Verlet.Poly.cloth()
			*	@param {object} opt
			*	@param [Array] dots
			*	@param [Array] cons
	...
	this.Interact = {
		...
		move : function(dots,color) {
			...
			let pointOffsetX = null, 
				...
				// changing document.body.onkeydown to window.onkeydow because electron 
				// does not respond to the document
...
/* 	
*	Render Given Objects With renderDots() and renderLines()
*	@method Verlet.renderDots()
*	@param [Array] dots
*	@param [Array] cons
*/
Verlet.prototype.superRender = function (dots,cons,opt) {
	...
	let renderDotsAsBox = option.renderDotsAsBox;
